...
export default class ProductsController {
  ...
  async updateStock({ params, request, response, auth }: HttpContext) {
    ...
    const product = await this.productService.updateStock(params.id, Number(stock), userId)
  ...
  async update({ params, request, response, auth }: HttpContext) {
    const payload = await request.validateUsing(productValidator)
    const userId = auth.user?.id
    const product = await this.productService.updateProduct(params.id, payload, userId)
    return response.ok(product)
  }
